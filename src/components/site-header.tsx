...
export function SiteHeader({ gmEnabled }: SiteHeaderProps) {
  return (
    <header className="sticky top-0 z-40 border-b marr-hairline bg-[#0d0c09]/86 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-3 py-3.5 sm:px-8">
        <Link href="/" className="group flex min-w-0 items-center gap-3" aria-label="Marr Interactive home">
        ...
        <nav className="flex shrink-0 items-center gap-1 text-sm text-stone-300 sm:gap-2">
          <Link
            href="/map"
            className="rounded border border-transparent px-2.5 py-2 transition hover:border-[#c9924a]/30 hover:bg-[#c9924a]/10 hover:text-[#eee7d7] sm:px-3"
          >
            Map
          </Link>
          <Link
            href="/gm"
            className={`rounded border px-2.5 py-2 transition sm:px-3 ${
              gmEnabled
          ...
          {gmEnabled ? (
            <form action={logoutGmMode}>
              <button
                type="submit"
                className="rounded border border-[#8b3a2f]/55 bg-[#8b3a2f]/18 px-2.5 py-2 text-[#f1d1c8] transition hover:bg-[#8b3a2f]/26 sm:px-3"
              >
                Lock
              </button>
            </form>
